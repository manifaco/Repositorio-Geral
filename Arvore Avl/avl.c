...
AVL* getSmallerAVL(AVL* avl){
	if (!avl || !avl->left) return avl;
	avl = getSmallerAVL(leftAVL(avl));
}

int deleteAVL(AVL* avl, info* inf){
	if(!avl) return 0;
	else {
		AVL* newNode = createAVLNode(newInf);
		AVL* auxAVL = avl;
		auxAVL = searchAVLNode(auxAVL,newNode);
		if (auxAVL) printf("auxAVL value:%i\n", auxAVL->info->value);
		AVL* auxAVLFather = searchAVLFather(avl,auxAVL);
		if (auxAVLFather) printf("auxAVLFather value:%i\n", auxAVLFather->info->value);
		printpreOrder(auxAVL);
		puts("86");
		
		if (auxAVL->left==NULL && auxAVL->right==NULL){
			if(auxAVL->info == avl->info){
				avl->right = NULL;
				avl->left = NULL;
				free(auxAVL);
				return 1;
				...
				printf("Pai:%i\n", auxAVLFather->info->value);
				if(auxAVLFather > auxAVL) auxAVLFather->left = NULL;
				else auxAVLFather->right = NULL;
				free(auxAVL);
				return 1;
			}
		}
		...
		if(auxAVL->left!=NULL || auxAVL->right!=NULL){
			if(auxAVL->left!=NULL && auxAVL->right==NULL){
				AVL* aux = auxAVL->left;
				auxAVL->info = auxAVL->left->info;
				auxAVL->left = auxAVL->left->left;
				auxAVL->right = auxAVL->left->right;
				free (aux);
				return 1;
			}
			
			if(auxAVL->right && !auxAVL->left){
				AVL*aux = auxAVL->right;
				auxAVL->info = auxAVL->right->info;
				auxAVL->left = auxAVL->right->left;
				auxAVL->right = auxAVL->right->right;
				free(aux);
				return 1;
			}
			
			if(auxAVL->left && auxAVL->right){
			AVL* smaller;
			AVL* aux;
			smaller = getSmallerNode(leftAVL(auxAVL));
			aux = searchAVLFather(auxAVL,smaller);
			aux->left = NULL;
			auxAVL->info = smaller->info;
			free(smaller);
			}
		}
		return 1;
